...
def clamp_condition_start_datetime(c):
  start = datetime.strptime(c["condition_start_datetime"], "%Y-%m-%d %H:%M:%S")
...
def run(argv=None, save_main_session=True):
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    for tbl in ['person', 'measurement', 'condition_occurrence']:
      ...
      for site in ['nyc', 'pitt']:
        if known_args.from_bigquery:
          ...
          ehr_inputs[tbl][site] = (
              p
              | f"read {site}_{tbl}" >> ReadFromText(f"../test_data/{site}/{tbl}.json")
    ...
    combined_by_domain["measurement"] = (combined_by_domain["measurement"], extracted_meas_rows) | beam.Flatten()
    ...
    person_id_blacklist = (
        ...
        | "generate the person ID blacklist" >> beam.Filter(lambda pid: int(pid) % 2 == 0)
    ...
    for (domain, data) in combined_by_domain.items():
      combined_by_domain[domain] = (
          data
          | beam.Filter(filter_by_id, blacklist=pvalue.AsSingleton(person_id_blacklist))
      )
    ...
    combined_by_domain['measurement'] = (
      ...
      | beam.Map(lambda m: ((m["person_id"], m["measurement_concept_id"]), m))
    ...
    for domain, data in combined_by_domain.items():
      data | f"output for {domain}" >> beam.io.WriteToText(f"out/{domain}.txt")
